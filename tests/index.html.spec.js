...
test.describe("index.html", () => {
  ...
  test('O primeiro link deve levar para uma página sobre listas', async ({ page }) => {
    await page.goto(`file://${process.cwd()}/index.html`);
    const link = page.locator("main > h1+p > a:first-child");
    await expect(link).toHaveAttribute("href", "listas.html");
    await expect(link).toHaveText("listas");
  });

  test('O segundo link deve levar para uma página sobre tabelas', async ({ page }) => {
    await page.goto(`file://${process.cwd()}/index.html`);
